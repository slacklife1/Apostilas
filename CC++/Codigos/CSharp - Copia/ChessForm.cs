...
namespace Chess
{
	public class ChessForm : System.Windows.Forms.Form
	{
		...
		public void init()
		{
			...
			this.Name = "ChessForm";
			this.Text = "Chess v1.0 by Tarýk Cörüt";
		...
		public int play(int i, int j)
		{
			...
			if(cl == 1)
			{
					...
					case 1:
						...
						if(cs2.move(brd,i,j)==1)
						{
							...
							if(king2.isChecked(b)==1)
							{
								...
								cl=0;
								MessageBox.Show("You cannot play that! Your will be mate");
								return 0;
							}
							lstr="R";
							pb[x1,y1].Image=null;
				...
				if(king.isChecked(brd)==1)
				{
					...
					{
						lstr=str2+" Checked "+lstr + " " + lstr3 + (y1+1).ToString() + " To " +lstr2 + (j+1).ToString();
						this.lb.Items.AddRange(new object[] {lstr});
